...
public class CoffSection {
     ...
     */
    public CoffSection(OpenFile file, Coff coff, int headerOffset) throws EOFException {
        this.file = file;
        this.coff = coff;
        ...
        if (vaddr % Processor.pageSize != 0 || size < 0
                || initialized && (contentOffset < 0 || contentOffset + size > file.length())) {
            Lib.debug(dbgCoffSection, "\tinvalid section addresses: " + "vaddr=" + vaddr + " size=" + size
    ...
    /**
     * Test whether this section is initialized. Loading a page from an initialized
     * section requires a disk access, while loading a page from an uninitialized
     * section requires only zero-filling the page.
     *
    ...
    public void loadPage(int spn, int ppn) {
        ...
        if (!initialized)
            ...
            /**
             * initlen = size % pageSize; Bug identified by Steven Schlansker 3/20/08 Bug
             * fix by Michael Rauser
             */
